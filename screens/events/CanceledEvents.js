...
export default function CanceledEvents({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      async function getData() {
        setLoading(true);
        const response = await getEvents(limitEvents, "4");
  ...
  const handleLoadMore = async () => {
    ...
    setLoading(true);
    const response = await getMoreEvents(limitEvents, startEvent, "4");
    if (response.statusResponse) {
      setStartEvent(response.startEvent);
