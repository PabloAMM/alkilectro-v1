...
export default function CompletedEvents({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      async function getData() {
        setLoading(true);
        const response = await getEvents(limitEvents, "3");
  ...
  const handleLoadMore = async () => {
    ...
    setLoading(true);
    const response = await getMoreEvents(limitEvents, startEvent, "3");
    if (response.statusResponse) {
      setStartEvent(response.startEvent);
